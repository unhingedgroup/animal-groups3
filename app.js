...
const els = {
  ...
  categoryFront:$('categoryFront'), categoryBack:$('categoryBack'),
  starsFront:$('starsFront'), starsBack:$('starsBack'),
  emojiFront:$('emojiFront'), emojiBack:$('emojiBack'),
  frontLead:$('frontLead'), frontMain:$('frontMain'),
  backMain:$('backMain'), backSub:$('backSub'),
  gradeBar:$('gradeBar'), gotBtn:$('gotBtn'), missBtn:$('missBtn'),
  favoriteBtn:$('favoriteBtn'), hardBtn:$('hardBtn'), skipBtn:$('skipBtn'),
  ...
  roundDone:$('roundDone'), roundScore:$('roundScore'),
  roundMessage:$('roundMessage'), newRoundBtn:$('newRoundBtn'),
  practiceMissedBtn:$('practiceMissedBtn'), confetti:$('confetti')
};
...
let favorites = new Set(JSON.parse(localStorage.getItem('animal-favorites') || '[]'));

const catClass = c => ({
  'Mammal':'var(--mammal)',
  'Bird':'var(--bird)',
  'Reptile':'var(--reptile)',
  'Amphibian':'var(--amphibian)',
  'Sea life':'var(--sea)',
  'Invertebrate':'var(--invert)'
}[c] || '#666');
...
function getPool(){
  ...
  if(sourceMode === 'favorites'){
    pool = pool.filter(x => favorites.has(x.animal));
  } else if(sourceMode === 'hard'){
    pool = pool.filter(x => hardCards.has(x.animal));
  } else if(sourceMode === 'mammal'){
    pool = pool.filter(x => x.category === 'Mammal');
  } else if(sourceMode === 'bird'){
    pool = pool.filter(x => x.category === 'Bird');
  } else if(sourceMode === 'reptile'){
    pool = pool.filter(x => x.category === 'Reptile' || x.category === 'Amphibian');
  } else if(sourceMode === 'sea'){
    pool = pool.filter(x => x.category === 'Sea life');
  } else if(sourceMode === 'invert'){
    pool = pool.filter(x => x.category === 'Invertebrate');
  }
...
function startRound(customPool=null){
  let pool = customPool ? [...customPool] : getPool();

  // Empty saved decks should remain empty, not silently become "All Animals".
  if(pool.length === 0){
    round = [];
    index = 0;
    correct = 0;
    ...
    els.roundScore.textContent = '0 cards';
    els.roundMessage.textContent =
      sourceMode === 'favorites'
        ? 'No favorites yet. Add some with the ♡ Favorite button.'
        : sourceMode === 'hard'
        ? 'No hard cards yet.'
        : 'This deck is empty.';
    els.practiceMissedBtn.disabled = true;
    els.practiceMissedBtn.textContent = 'No missed cards';
    return;
  }

  pool = shuffle(pool);

  // All Animals = the full deck (112 cards).
  // Every other deck = up to 20 cards.
  const useFullDeck = !customPool && sourceMode === 'all';
  round = useFullDeck ? pool : pool.slice(0, Math.min(ROUND_SIZE, pool.length));
...
function updateSaveButtons(d){
  const isFavorite = favorites.has(d.animal);
  const isHard = hardCards.has(d.animal);

  els.favoriteBtn.textContent = isFavorite ? '♥ Favorite' : '♡ Favorite';
  els.favoriteBtn.setAttribute('aria-pressed', String(isFavorite));
  els.favoriteBtn.classList.toggle('active-save', isFavorite);

  els.hardBtn.textContent = isHard ? '★ Hard' : '☆ Hard';
  els.hardBtn.setAttribute('aria-pressed', String(isHard));
  els.hardBtn.classList.toggle('active-save', isHard);
}
...
function render(){
  ...
  els.card.classList.remove('flipped','shake');
  flipped = false;
  els.gradeBar.classList.add('hidden');
...
function finishRound(){
  ...
  els.practiceMissedBtn.textContent =
    missed.length ? `Practice ${missed.length} missed` : 'No missed cards';
}
...
els.favoriteBtn.addEventListener('click', () => {
  ...
  if(favorites.has(d.animal)){
    favorites.delete(d.animal);
  } else {
    favorites.add(d.animal);
  }

  saveFavorites();
  updateSaveButtons(d); // Do NOT re-render or flip/reset the card.
});
...
els.hardBtn.addEventListener('click', () => {
  ...
  if(hardCards.has(d.animal)){
    hardCards.delete(d.animal);
  } else {
    hardCards.add(d.animal);
  }
...
els.practiceMissedBtn.addEventListener('click', () => {
  if(missed.length) startRound(missed);
});

let startX = null;
els.scene.addEventListener('touchstart', e => {
